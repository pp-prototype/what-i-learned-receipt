...
4. 아래 마이그레이션을 파일명 순서대로 각각 복사해 실행한다.
   ...
   - [`202607240004_create_receipt_gallery.sql`](../supabase/migrations/202607240004_create_receipt_gallery.sql)
...
- 발행한 영수증 메타데이터와 비공개 `receipts` Storage 버킷
- 사용자 본인만 영수증과 이미지를 조회·저장하게 하는 RLS 정책
- 사용자당 영수증·Storage 객체 100개 및 이미지당 5MB 저장 한도
...
9. 발행 후 `갤러리` 탭에 새 영수증이 표시되는지 확인한다.
10. 갤러리 이미지를 선택했을 때 원본 영수증을 열 수 있는지 확인한다.
11. 두 단말기에서 같은 Google 계정으로 로그인하고 양쪽 세션이 유지되는지 확인한다.
12. 한 단말기에서 로그아웃해도 다른 단말기의 세션이 유지되는지 확인한다.
13. 다른 Google 계정으로 로그인했을 때 첫 계정의 프로젝트와 영수증이 조회되지 않는지 확인한다.
14. 한 계정에서 프로젝트를 50개보다 많이 생성할 수 없는지 확인한다.
